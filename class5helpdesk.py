...
def action():
    ...
    tk.Label(fram1,font=('aria',22),fg='red', text="Hello " + e1.get() +" please Select Below which problem's Solution you Want :",width =60,height=1).place(x=0,y=45) 
    e4 = tk.Entry(fram1)
    e5 = tk.Entry(fram1)
    ...
    variable.set(OptionList_sub[0])

    opt = tk.OptionMenu(master, variable, *OptionList_sub)
    opt.config(width=10, font=('Helvetica', 12))  
    opt.place(x=750,y=275)

    variable2 = tk.StringVar(master)
    variable2.set(OptionList_chap[0])

    opt = tk.OptionMenu(master, variable2, *OptionList_chap)
    opt.config(width=10, font=('Helvetica', 12))  
    opt.place(x=750,y=312)
    labelTest = tk.Label(text=f"Search Your Queries...", font=('Helvetica', 15), fg='red',bg='cyan')
    labelTest.place(x=670,y=395)
